...
## Session 2026-08-07/08 — 014-continuous-integration + 015-ci-test-timeout
...
Both features **done** and merged. `main` has CI for the first time, and `main` is **green** for the
first time (push run 31235303720, `RESULT: SUCCESS (10/10 stages passed)`).

- **014-continuous-integration** — merged as PR #9 (`0589e03`), deliberately with a red check, at the
  human's instruction, because `main` had no CI and no protection at the time so a red check was
  information rather than a regression, and landing the workflow let 015 get a real check without a
  throwaway branch. 8 of 9 tasks `[X]`, three `code-reviewer` passes all APPROVED. **T007 (branch
  protection) is the sole open item and remains unchecked — a repo setting no agent can apply.**
- **015-ci-test-timeout** — merged as PR #10 (`d3d1c00`). All 22 checkboxes `[X]`. Registered,
  spec'd, and fixed entirely within this session because 014's CI caught it within minutes of
  existing: a real defect that passed 630/630 locally.

**The headline**: CI paid for itself immediately. It found `LoginScreen.test.tsx`'s first test
exceeding jest's 5000ms per-test timeout on a runner while green locally — exactly the "green tests,
broken app" class this repo had documented but never had a mechanical guard against. No synthetic
broken commit was ever needed to prove the fail path; a genuine bug did it.

**Three rounds to root-cause it, each recorded so the dead ends are never retried**: (1) the
`@expo/vector-icons` async font load — a real win (44 `act()` warnings → 0, −16% local test time)
that did not move the timeout; (2) jest worker oversubscription — a real contributor (69ms @ 1 worker
vs 308ms @ 13), `--runInBand` kept, but it only reached 3885ms of 5000ms; (3) **cold jest transform
cache** — the dominant term (warm 147ms vs 1666ms cleared). Eliminated by measurement: setup-file
module warming (doubled total test time and *slowed* the target test) and a canary test (imports
evaluate at module load, outside any test's clock).

**Two methodological lessons worth carrying forward.** Suite-level timing hid the bug: from a 6.759s
suite total the target test was inferred at ~600ms when it was 3885ms — wrong by ~6x — which is why
the workflow now dumps logs on passing runs and CI runs jest `--verbose`. And GitHub Actions caches
created on a feature branch do NOT propagate to `main`, so the first run on `main` after any keyed
file changes is always cold (measured 3792ms there) — that cold case is exactly what the
human-authorized `--testTimeout=15000` exists to absorb.

**Corrections made to standing notes**: 010-registration-redesign is merged (PR #7) — the "unmerged,
no open PR" claim in 012's and 014's notes was stale. And a full unflagged `./init.sh` reports
`10/10`, not `8/8` (8 numbered stages, but stage 8 records three bundle exports separately).

**Still parked**: 012-home-visual-alignment (`spec_ready`, at its own approval gate — its spec docs
landed on `main` via 014's PR), 013-auth-gate-regression-guard (`pending`; its jest guards become
enforceable the moment T007 is applied), plus 002/003/007/009/011.
